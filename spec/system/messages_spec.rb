...
RSpec.describe "メッセージ投稿機能", type: :system do
  before do
    # 中間テーブルを作成して、usersテーブルとroomsテーブルのレコード”""も”""作成する
  ...
  context "投稿に失敗したとき" do
    it "送る値が空の為、メッセージの送信に失敗する" do
      ...
      # 作成されたチャットルームへ遷移する
      # roomについても上と同様。@room_userに紐づくroomが生成されているため、@room_user.roomというアソシエーションの記述で取得できる
  ...
  context "投稿に成功したとき" do
    it "テキストの投稿に成功すると、投稿の一覧に遷移して、投稿した内容が表示されている" do
      # サインインする
      sign_in(@room_user.user)
    ...
    it "チャットルームを削除すると、関連するメッセージが全て削除されている" do
      ...
      # メッセージ情報を5つDBに追加する
      FactoryBot.create_list(:message, 5, content: "hello", room_id: @room_user.room.id, user_id: @room_user.user.id)
